...
def extract_info_nii(path):
    info = {"contrast": "", "dose": "", "filter": "", "direction": ""}
    try:
        img = nib.load(path)
        hdr = img.header
        descrip = str(hdr.get("descrip", b"")).lower()
        intent_name = str(hdr.get("intent_name", b"")).lower()
        combined = f"{descrip} {intent_name} {path.lower()}"
        if re.search(r"(contrast|non[- ]?contrast|with|without)", combined):
            info["contrast"] = combined
        if re.search(r"(dose|kvp|exposure)", combined):
            info["dose"] = combined
        if re.search(r"(filter|kernel|smooth|sharp|bone|lung|standard)", combined):
            info["filter"] = combined
        if re.search(r"(axial|coronal|sagittal|3d)", combined):
            info["direction"] = combined
    except:
        pass
    return info

def classify(info, filename, path):
    fname = f"{filename.lower()} {os.path.basename(os.path.dirname(path)).lower()}"
    combined = f"{info['contrast']} {info['dose']} {info['filter']} {info['direction']} {fname}"

    if re.search(r"(contrast|non[- ]?contrast|with|without)", combined):
        return "contrast"
    elif re.search(r"(low[- ]?dose|high[- ]?dose|kvp)", combined):
        return "dose"
    elif re.search(r"(bone|lung|standard|sharp|smooth|kernel)", combined):
        return "filter"
    elif re.search(r"(axial|coronal|sagittal|3d)", combined):
        return "direction"
    else:
        return "others"
    ...
    for root, _, files in os.walk(dataset_dir):
        for f in files:
            ...
            if is_dicom(src):
                ...
                info = extract_info_nii(src)
                file_type = "NIfTI"
            else:
                continue

            total_files += 1
            category = classify(info, f, src)
...
if not datasets:
    print("❌ No datasets found in", base_path)
else:
    for ds in datasets:
        arrange_dataset(ds)
